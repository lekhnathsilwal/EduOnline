...
?>
    ...
    @else
        <div style="margin-top:20px;" class="container">
            <div class="row">
                <div class="col-md-12">
                    <form action="{{url('student/search-teacher')}}" class="search" method="POST"  role="search">
                        @csrf
                ...
                @foreach ($posts as $post)
                    <div class="well">
                        <div class="post_head">
                            ...
                            <a href="{{url('show-teacher-profile/'.$post->user->id)}}"><h4 style="margin-left: 70px; padding-top:5px; text-transform: capitalize;">{{$post->user->name}}</h4></a>
                        </div>
...
$('.like-btn').on('click', function(){
  ...
  document.getElementById("aaa").innerHTML = post_id;
  var urlLike = '{{ route('student.dashboard') }}';
  $.ajax({
  	url: urlLike,
  	type: 'post',
  	data: {
